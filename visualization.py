...
def is_interesting(serie):
    ...
    if np.all(np.linalg.norm(np.array([x,y]),axis=0)>0.45):
        return False
...
def plot_stability(plot_handle):
    ...
    plot_handle.set_xlabel("Initial position")
    plot_handle.set_ylabel("Initial velocity")
...
plot_energy(energy_graph,sequential_data)
